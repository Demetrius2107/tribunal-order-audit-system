 ...
 * <p>设计要点：</p>
 * <ol>
 *   <li>每个状态只允许迁移到  中定义的目标状态</li>
 *   <li>状态重复回传（行业通用做法"当前订单状态已经同步过"）由「目标状态不在 TRANSITIONS 中」天然拦截</li>
 ...
 *   <li>部分发货（行业通用做法 PART_SHIPMENT）</li>
 *   <li>冬储单终止（行业通用做法 ORDER_STOP，行业特有，可选）</li>
 *   <li>思考：哪些状态允许"重复触发"（行业通用做法 CONFIRM_AN_ORDER 允许 005/301 前置）</li>
 * </ul>
 */
public enum OrderStatus {
     ...
     * <ol>
     *   <li>是否需要"前置状态允许有多个"的扩展（如发货完成允许多个前置状态）</li>
     *   <li>是否引入"迁移动作"（如迁移到 SHIPPED 前必须执行库存/物流校验）——
     *       可借鉴策略模式把迁移动作挂到每个枚举值上</li>
     * </ol>
     *
     * @param target 目标状态
     * @return true 允许迁移
